...
For each group `i` of `N`, call the `question` tool **once** with the commit card inline in the question text.

**Commit card** (embed in the `question` field):
...
question({
  questions: [{
    question: `${commitCard}\n\n[${i}/${N}] Commit this group?`,
    header: `${type}(${scope})`,  // ≤30 chars
    options: [
      {
