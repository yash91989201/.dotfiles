...
Choose tools by the shape of the task, not by habit. Use broad search for discovery, documentation indexes for authoritative technical references, crawlers for multi-page sites, extraction tools for specific URLs, and sandboxed processing when raw output would be large or repetitive.
